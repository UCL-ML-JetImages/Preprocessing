...
// Open files
	TFile *file = TFile::Open("W_Signal.root");	// switch file between signal and background here
	TTree *tree = (TTree*)file -> Get("FlatSubstructureJetTree"); 
	TCanvas *can1 = new TCanvas ("can1", "Eta Phi Plot", 600, 400); //creates canvas
...
// Fill histo1 with clusters	
	...
	for (int i = 0; i<tree_size; i++) {
		...
		while ((fjet_fatjet_dRmatched_particle_flavor == Wboson) || (fjet_fatjet_dRmatched_particle_flavor == -Wboson)) {	//PGID, comment out for background file	
			...
			for (int q = 0; q<clus_eta->size(); q++) {					
				...
				//while (clus_energy->at(q) != max) {
					...
					histo1 -> Fill(clus_Nfin, clus_Pfin, clus_1_rawE);	// switch out third value to weight differently				
						clus_hits++;
					}
...
// Plot histo1
	...
	histo1 -> GetZaxis() -> SetTitleOffset(1.35);
   	...
   	histo1 -> Draw("COLZ");
   	can1 -> SaveAs ("LSB_50by50_ClusRawE_NoScale.png");
	return 0;
}			
